...
if test -f "$_JINC/jni.h"; then
		...
		AC_DEFINE_UNQUOTED(JNI_HEADER, ["$_JINC/jni.h"], ["JNI Header File"])
else
        _JTOPDIR=`echo "$_JTOPDIR" | sed -e 's:/[[^/]]*$::'`
        if test -f "$_JTOPDIR/include/jni.h"; then
                JNI_INCLUDE_DIRS="$JNI_INCLUDE_DIRS $_JTOPDIR/include"
				AC_DEFINE_UNQUOTED(JNI_HEADER, ["$_JTOPDIR/include/jni.h"], ["JNI Header File"])
		elif test -f "$_JTOPDIR/Headers/jni.h"; then
                JNI_INCLUDE_DIRS="$JNI_INCLUDE_DIRS $_JTOPDIR/Headers"
				AC_DEFINE_UNQUOTED(JNI_HEADER, ["$_JTOPDIR/Headers/jni.h"], ["JNI Header File"])
        else
